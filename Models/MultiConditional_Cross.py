...
class ConditionText(nn.Module):
    ...
    def __init__(self, d_model, n_head=8, d_ff=2048, dropout=0.1, N=1, k=49):
        ...
        self.attIT = MultiHeadAttention(n_head, d_model, dropout)
    ...
    def forward(self, hiddents, V, T):
        ...
        conIT = torch.mean(self.attIT(DeltaV, T_, T_), dim=1, keepdim=True)
        conIT = conIT.expand_as(conI)

        return conI, conT, conIT
...
class Decoder(nn.Module):
    def __init__(self, embed_size, vocab_size, hidden_size, N=1, v_size=49):
        ...
        self.mlp = nn.Linear(hidden_size * 5, vocab_size)
    ...
    def forward(self, V, T, captions, basic_model, states=None):
        ...
        conI, conT, conIT = self.condition(hiddens, V, T)
        ctx = self.attention(hiddens, curr_vf, curr_vf)
        output = torch.cat([hiddens, ctx, conI, conT, conIT], dim=2)
