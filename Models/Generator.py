...
class Generator(object):
    ...
    def translate_batch(self, images, image_concepts):
        ...
        def collect_active_part(beamed_tensor, curr_active_inst_idx, n_prev_active_inst, n_bm):
            ...
            elif beamed_tensor.dim() == 4:
                _, d_hs, d_hs_2, d_hs_3 = beamed_tensor.size()
                new_shape = (n_curr_active_inst * n_bm, d_hs, d_hs_2, d_hs_3)
            else:
        ...
        with torch.no_grad():
            ...
            n_inst, idx, len_s, d_h = V.size()
            V = V.repeat(1, n_bm, 1, 1).view(n_inst * n_bm, idx, len_s, d_h)

            if T.dim() == 4:
                n_inst, k, len_s, d_h = T.size()
                T = T.repeat(1, n_bm, 1, 1).view(n_inst * n_bm, k, len_s, d_h)
            else:
                n_inst, len_s, d_h = T.size()
            ...
            # -- Decode
            for len_dec_seq in range(1, self.args.max_length):
